...
// Default connectionMethod
  if (!m_settings.contains(OPTION_NODE_CONNECTION_METHOD))
  {
      QWriteLocker lock(&m_lock);
#ifdef Q_OS_WIN
      m_settings.insert(OPTION_NODE_CONNECTION_METHOD, static_cast<int>(ConnectionMethod::AUTO));
#else
      m_settings.insert(OPTION_NODE_CONNECTION_METHOD, static_cast<int>(ConnectionMethod::EMBEDDED));
#endif
      saveSettings();
  }
  notifyObservers();
...
ConnectionMethod Settings::getConnectionMethod() const {
  QReadLocker lock(&m_lock);
  #ifdef Q_OS_WIN
      return m_settings.contains(OPTION_NODE_CONNECTION_METHOD) ? static_cast<ConnectionMethod>(m_settings.value(OPTION_NODE_CONNECTION_METHOD).toInt()) : ConnectionMethod::AUTO;
  #else
	  return m_settings.contains(OPTION_NODE_CONNECTION_METHOD) ? static_cast<ConnectionMethod>(m_settings.value(OPTION_NODE_CONNECTION_METHOD).toInt()) : ConnectionMethod::EMBEDDED; // Change default to remote
  #endif
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    if (_enable) {
      ...
      autorunSettings.setValue(keyName, appPath);
    } else {
