...
// seed
#include "Gui/Seed/RestoreFromMnemonicSeedDialog.h"
...
// Set this to available only for Win
#ifdef Q_OS_WIN
    m_ui->m_minimizeToTrayAction->setEnabled(true);
	m_ui->m_closeToTrayAction->setEnabled(true);
	m_ui->m_autostartAction->setEnabled(true);
#else
    m_ui->m_minimizeToTrayAction->setEnabled(false);
	m_ui->m_closeToTrayAction->setEnabled(false);
	m_ui->m_autostartAction->setEnabled(false);   
#endif
...
// Clone from importKey.
// TODO Coding restoreFromMnemonicSeed
void MainWindow::restoreFromMnemonicSeed() {
  ...
  if (dlg.exec() == QDialog::Accepted) {
    ...
    std::string mnemonicPhrase = mnemonicString.toStdString();
	Crypto::SecretKey private_spend_key;
	AccountKeys keys;
	if(crypto::ElectrumWords::words_to_bytes(mnemonicString.toStdString(), private_spend_key, seed_language)) {
		Crypto::PublicKey public_spend_key;
		...
		keccak((uint8_t *)&private_spend_key, sizeof(Crypto::SecretKey), (uint8_t *)&second, sizeof(Crypto::SecretKey));
		...
		keys.viewKeys.secretKey = private_view_key;
		keys.viewKeys.publicKey = public_view_key;
		...
		Settings::instance().setWalletFile(filePath);
 		...
 		if (walletAdapter->createWithKeys(filePath, keys) == IWalletAdapter::INIT_SUCCESS) {
		  walletAdapter->save(CryptoNote::WalletSaveLevel::SAVE_ALL, true);
		} else {
		  Settings::instance().setWalletFile(oldWalletFile);
		} 
